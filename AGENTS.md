...
## 1.1 死循环防范规则（防止 Claude 工具调用死循环）

- **根本原则**：写文件成功后直接用文字回复用户，禁止用 bash echo 或其他工具调用来”验证”写文件是否成功。Write/Edit 工具失败会直接报错——无报错即成功。
- **禁止行为**：
  - 禁止用 `echo “OK”`、`echo “DONE”`、`echo “.”`、`echo “READY”` 等无实际意义的 bash 命令来报告状态或占位。
  - 禁止在无实际问题需要验证时连续执行 3 个以上同模式工具调用。
  - 禁止用工具调用来”填补等待”—如果需要等待用户指令，直接文字告知即可。
- **状态报告规则**：完成任务后的状态报告和总结必须用文字完成，不得用 bash echo 代替。
- **触发信号**：如果发现自己连续 3 次执行了 `echo “X”` 模式（X 为任意字符串），立即停止所有工具调用，转为文字回复。
- **自我进化机制（Dead Loop Self-Healing）**：如果本规则被触发（即死循环实际发生），在解决用户问题之前，必须先执行以下自我修复流程：
  1. 标记错误：在 `memory/memory/feedback_no_tool_spam.md` 中记录本次循环的触发条件和上下文
  2. 分析根因：识别导致本次循环的具体工具调用序列（是 Write/Edit 后的”确认焦虑”？是 VM 不可用时的”重试循环”？还是任务完成后的”等待填充”？）
  3. 升级规则：如果 §1.1 已有规则无法阻止本次循环，说明现有约束存在漏洞，必须在本段增加新的禁止条款或触发条件。升级后的规则要能**精确匹配**本次实际发生的触发模式
  4. 追加到 AGENTS.md：更新本段规则后，同步更新 CLAUDE.md（已为软链接时自动生效）
- **自我验证**：规则升级后，在文字回复用户之前，检查本次会话中是否已经出现 ≥3 次同模式工具调用——如果已出现，说明升级无效，进一步强化约束条件

## 1.2 记忆与经验复用规则（防止忘记已学到的教训）

- **记忆目录**：`memory/memory/` 是本仓库的用户记忆目录。每次会话开始时应读取 `memory/memory/MEMORY.md` 和其中的记忆文件，确保前次会话的教训被载入。
- **记忆成员**：记忆文件如下，每次读 MEMORY.md 时应同时加载其中引用的所有 `.md` 文件：
  - `memory/memory/user_profile.md`：用户画像
  - `memory/memory/user_quant_background.md`：用户有 10+ 次失败量化项目经历
  - `memory/memory/user_project_vision.md`：项目目标是后端组织能力 + Agent 调度/学习/反思能力为核心，不是前端展示
  - `memory/memory/feedback_no_tool_spam.md`：用户明确反对用工具调用代替文字回复
- **复习要求**：每次会话开始或收到新指令后，检查 MEMORY.md 是否有更新。如果上述记忆尚未加载，先加载再继续。
